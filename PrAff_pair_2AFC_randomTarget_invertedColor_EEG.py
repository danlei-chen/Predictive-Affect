# Psuedo_randomized target location
...
def giveNewValueIn1D(indexList, newValue, newList1D):
    count = 0
    for eachIndex in indexList:
        path = newValue[count]         
        newList1D[eachIndex[0]] = path
        count += 1
    return newList1D
...
#     design['base']['stimPairTypeID'] = param['stimPairTypeID']
    ...
    #                 print('repetition:',m)  
                ...
                # randomize triplet order across block.
        ...
        #              once there are no repeats, append to the existing block.
            ...
            design['block'][n]['greypatch_positionLorR'] = ['']*len(design['block'][n]['stimuli'])
            for im in design['block'][n]['stimuli']:
                greyp_patch_LorR = ['L','R']*int(param['total_num_rep']/2)
                if param['total_num_rep'] %2 != 0:
                    greyp_patch_LorR.extend(greyp_patch_LorR[len(greyp_patch_LorR)-1])
                randomizer = np.random.permutation(len(greyp_patch_LorR))
                greyp_patch_LorR = [greyp_patch_LorR[i] for i in randomizer]

                indexList = np.argwhere(np.array(design['block'][n]['stimuli'])==im)
                design['block'][n]['greypatch_positionLorR'] = giveNewValueIn1D(indexList, greyp_patch_LorR[(0+n*param['rep_per_block']):((n+1)*param['rep_per_block'])], design['block'][n]['greypatch_positionLorR'])


            design['block'][n]['greypatch_positionx'] = ['']*len(design['block'][n]['stimuli'])
            design['block'][n]['greypatch_positiony'] = ['']*len(design['block'][n]['stimuli'])
            grey_x=[]
            grey_y=[]   
            for trial in range(len(design['block'][n]['greypatch_positionLorR'])):
                x=param['img_x']/2
                y=param['img_y']/2
                if design['block'][n]['greypatch_positionLorR'][trial] == 'L':
                    #left
                    x=random.uniform(-(param['img_x']/2)+param['patch_length'][0],0-param['patch_length'][1])
                elif design['block'][n]['greypatch_positionLorR'][trial] == 'R':
                    #right
            ...
            design['block'][n]['greypatch_positionx'] = grey_x
            design['block'][n]['greypatch_positiony'] = grey_y
